...
export default function UserDrawer({
  ...
  return (
    <div className="fixed right-0 top-0 z-[150] h-full w-full sm:w-[500px] pointer-events-auto">
      <div
        className={`h-full bg-zinc-950 border-l border-white/10 shadow-2xl transition-transform duration-300 ease-out ${
      ...
      >
        ...
        <div className="p-6 border-b border-white/5 flex justify-between items-center bg-zinc-900/50">
          ...
          <button
            onClick={onClose}
            className="h-10 w-10 rounded-xl flex items-center justify-center text-zinc-500 hover:text-white hover:bg-white/5 transition-all"
            type="button"
          >
            <X size={20} />
          </button>
          ...
          <div className="rounded-3xl bg-white/5 border border-white/10 p-5 space-y-5">
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <div className="text-xs text-zinc-500 uppercase font-bold tracking-tighter italic">
          ...
          {!readOnly && (
            <div className="grid grid-cols-2 gap-4">
              <button
                ...
                className="flex items-center justify-center gap-2 py-3 rounded-xl bg-white/10 hover:bg-white/20 font-bold transition-all border border-white/10 text-white"
                type="button"
              >
                <Pencil size={18} />
                Chỉnh sửa
...
function InfoItem({ icon: Icon, label, value, className = "" }) {
  return (
    <div className="rounded-2xl bg-white/[0.03] border border-white/5 p-3">
      ...
      <div className={`text-zinc-200 ${className}`}>{value || "N/A"}</div>
    </div>
  );
}